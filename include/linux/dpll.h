...
struct dpll_device_ops {
	...
	int (*get_source_type)(struct dpll_device *dpll, int sma);
 ...
 * @dpll: pointer to dpll object that owns the pin
...
/*
 * dpll_muxed_pin_free - free memory allocated for a pin
 * @parent_pin: pointer to object that owns the pin
 * @pin: pointer to allocated pin object being freed
 *
 * Free memory of deregistered pin object.
 * Shared pins must be deregistered from all dpll devices before freeing them,
 * otherwise memory won't be freed.
 */
